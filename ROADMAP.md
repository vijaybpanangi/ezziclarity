...
- **Books gallery — flip titles to live** (`v3.7.0` shipped the gallery). Each Books-page card is now a `.book-card` in framework state (branded placeholder cover + "Coming soon / Notify me"). When a title goes live: (1) drop its real cover into `assets/images/books/<slug>-cover.jpg` and swap the `.book-cover--placeholder` div for `<img class="book-cover" …>`; (2) uncomment the buy button in that card's `<!-- FLIP TO LIVE -->` block and set the real Amazon (Associates) URL; (3) update the status pill; (4) add a schema.org `Book` JSON-LD node (name, author → `#founder`, image, `offers` → Amazon) to that page's `@graph`. Repeat in all three language trees. Instructions live in each card's HTML comment.
